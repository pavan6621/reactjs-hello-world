...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button,  CardContent, Card,
  Dialog,
  DialogTitle,
  DialogContent,
  DialogActions,
...
No of pages
<ArrowDropDownIcon sx={{ color:'black',position: 'relative',left: 5}}  /> 
</Button>
 <Button   sx={{ color:'black',marginLeft:'10px',marginRight:'10px', 
   background: 'linear-gradient(to right, lavender 0%, lavender 20%, white 20%, white 80%, lavender 80%, lavender 100%)',
    
   
   }}>
<ArrowLeftIcon sx={{ bcolor:'black',position: 'relative',right:10}} />
 1 to 9
< ArrowRightIcon sx={{ color:'black',position: 'relative',left: 10}} />
</Button>
    ...
    <Table  >
      ...
      {data.map((row) => (
          <TableRow  key={row.id} >
            ...
            <TableCell> {editingId === row.id ? (
                  <input type="text" value={row.title} onChange={e => {  const updatedData = { ...row, title: e.target.value }; saveUpdatedRow(row.id, updatedData); }}/>
                ) : (
                  row.title
                )}</TableCell>
            <TableCell>  {editingId === row.id ? (
                  <input type="text" value={row.body} onChange={e => {  const updatedData = { ...row, body: e.target.value }; saveUpdatedRow(row.id, updatedData); }}/>
                ) : (
                  row.body
                )}</TableCell>
            <TableCell > {editingId === row.id ? (
                  <button onClick={() => saveUpdatedRow(row.id, row)}><SaveAltIcon /></button>
                ) : (
                  <>
                   <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Button   variant="outlined"
                    color="secondary" onClick={() => setEditingId(row.id)}><EditIcon /></Button>
                    <Button
                    variant="outlined"
                    color="secondary"
